		...
		Grid.createGridFromAStarMap = function (texture) {
			...
			for (var w = 0; w < textureWidth; w++) {
				...
				for (var h = 0; h < textureHeight; h++) {
					...
					if (r == 255 && g == 255 && b == 255 && a == 255) colaStarArr[h] = 1;else {
						colaStarArr[h] = 0;
					}
				}
			};
 ...
 */
	...
	var Util = function () {
		...
		Util.biBacktrace = function (nodeA, nodeB) {
			var pathA = Util.backtrace(nodeA),
			    pathB = Util.backtrace(nodeB);
 ...
 */
  ...
  */
		__proto.findPath = function (startX, startY, endX, endY, grid) {
			...
			var search = function search(node, g, cutoff, route, depth) {
				nodesVisited++;
