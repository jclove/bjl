    ...
    (function prelude(modules, cache, entries) {
        ...
        if ((typeof module === "undefined" ? "undefined" : _typeof(module)) === "object" && module && module.exports) module.exports = protobuf;
            ...
            function codegen() {
                ...
                function gen() {
                    ...
                    if (src.length) {
                        ...
                        if (blockOpenRe.test(prev)) level = ++indent; // keep
                        else if (branchRe.test(prev)) ++level; // once
                        ...
                        // block close
                        if (blockCloseRe.test(line)) level = --indent;
                    }
                    for (i = 0; i < level; ++i) {
                        line = "\t" + line;
                    }src.push(line);
                ...
                function eof(name, scope) {
                    ...
                    if (codegen.verbose) console.log("--- codegen ---\n" + source.replace(/^/mg, "> ").replace(/\t/g, "  ")); // eslint-disable-line no-console
                    var keys = Object.keys(scope || (scope = {}));
                    return Function.apply(null, keys.concat("return " + source)).apply(null, keys.map(function (key) {
                        return scope[key];
            ...
            function sprintf(format) {
                ...
                if (i !== args.length) throw Error("argument count mismatch");
                return format;
            }

            codegen.sprintf = sprintf;
            codegen.supported = false;try {
                codegen.supported = codegen("a", "b")("return a-b").eof()(2, 1) === 1;
            ...
            function Class(type, ctor) {
                if (!Type) Type = require(34);

                if (!(type instanceof Type)) throw TypeError("type must be a Type");

                if (ctor) {
                    if (typeof ctor !== "function") throw TypeError("ctor must be a function");
                } else ctor = Class.generate(type).eof(type.name); // named constructor function (codegen is required anyway)
                ...
                for (; i < /* initializes */type.fieldsArray.length; ++i) {
                    ...
                    ctor.prototype[type._fieldsArray[i].name] = Array.isArray(type._fieldsArray[i].resolve().defaultValue) ? util.emptyArray : util.isObject(type._fieldsArray[i].defaultValue) && !type._fieldsArray[i].long ? util.emptyObject : type._fieldsArray[i].defaultValue; // if a long, it is frozen when initialized
                }
            ...
            Class.generate = function generate(type) {
                ...
                for (var i = 0, field; i < type.fieldsArray.length; ++i)
                    if ((field = type._fieldsArray[i]).map) gen
                        ...
                        ("this%s=[]", util.safeProp(field.name));
                */
                return gen("if(p){")("for(var ks=Object.keys(p),i=0;i<ks.length;++i)")("this[ks[i]]=p[ks[i]];")("}");
            ...
            converter.toObject = function toObject(mtype) {
                ...
                for (; i < fields.length; ++i) {
                    if (fields[i].resolve().repeated) repeatedFields.push(fields[i]);else if (fields[i].map) mapFields.push(fields[i]);else otherFields.push(fields[i]);
                ...
                for (i = 0, field; i < fields.length; ++i) {
                    var field = fields[i],
                        prop = util.safeProp(field.name);gen("if(m%s!==undefined&&m%s!==null&&m.hasOwnProperty(%j)){", prop, prop, field.name);
                    if (field.map) {
                        gen("d%s={}", prop)("for(var ks2=Object.keys(m%s),j=0;j<ks2.length;++j){", prop);
                        genValuePartial_toObject(gen, field, i, prop + "[ks2[j]]")("}");
                    } else if (field.repeated) {
                        gen("d%s=[]", prop)("for(var j=0;j<m%s.length;++j){", prop);
                        genValuePartial_toObject(gen, field, i, prop + "[j]")("}");
                    } else genValuePartial_toObject(gen, field, i, prop);
                    gen("}");
                }
                return gen("return d");
                /* eslint-enable no-unexpected-multiline, block-scoped-var, no-redeclare */
            ...
            function decoder(mtype) {
                /* eslint-disable no-unexpected-multiline */
                var gen = util.codegen("r", "l")("if(!(r instanceof Reader))")("r=Reader.create(r)")("var c=l===undefined?r.len:r.pos+l,m=new this.ctor")("while(r.pos<c){")("var t=r.uint32()");
                ...
                for (; i < /* initializes */mtype.fieldsArray.length; ++i) {
                    var field = mtype._fieldsArray[i].resolve(),
                        ...
                        ref = "m" + util.safeProp(field.name);gen("case %d:", field.id);

                    // Map fields
                    if (field.map) {
                        gen("r.skip().pos++") // assumes id 1 + key wireType
                        ("if(%s===util.emptyObject)", ref)("%s={}", ref)("var k=r.%s()", field.keyType)("r.pos++"); // assumes id 2 + value wireType
                        if (types.basic[type] === undefined) gen("%s[typeof k===\"object\"?util.longToHash(k):k]=types[%d].decode(r,r.uint32())", ref, i); // can't be groups
                        ...
                        if ((decoder.compat || field.packed) && types.packed[type] !== undefined) gen("if((t&7)===2){")("var c2=r.uint32()+r.pos")("while(r.pos<c2)")("%s.push(r.%s())", ref, type)("}else");

                        // Non-packed
                        if (types.basic[type] === undefined) gen(field.resolvedType.group ? "%s.push(types[%d].decode(r))" : "%s.push(types[%d].decode(r,r.uint32()))", ref, i);else gen("%s.push(r.%s())", ref, type);

                        // Non-repeated
                    } else if (types.basic[type] === undefined) gen(field.resolvedType.group ? "%s=types[%d].decode(r)" : "%s=types[%d].decode(r,r.uint32())", ref, i);else gen("%s=r.%s()", ref, type);
                    gen("break");
            ...
            function genTypePartial(gen, field, fieldIndex, ref) {
                return field.resolvedType.group ? gen("types[%d].encode(%s,w.uint32(%d)).uint32(%d)", fieldIndex, ref, (field.id << 3 | 3) >>> 0, (field.id << 3 | 4) >>> 0) : gen("types[%d].encode(%s,w.uint32(%d).fork()).ldelim()", fieldIndex, ref, (field.id << 3 | 2) >>> 0);
            }
            ...
            function encoder(mtype) {
                /* eslint-disable no-unexpected-multiline, block-scoped-var, no-redeclare */
                var gen = util.codegen("m", "w")("if(!w)")("w=Writer.create()");
                ...
                if (encoder.compat) fields = fields.slice().sort(compareFieldsById);
                ...
                for (var i = 0; i < fields.length; ++i) {
                    ...
                    // Map fields
                    if (field.map) {
                        gen("if(%s&&m.hasOwnProperty(%j)){", ref, field.name)("for(var ks=Object.keys(%s),i=0;i<ks.length;++i){", ref)("w.uint32(%d).fork().uint32(%d).%s(ks[i])", (field.id << 3 | 2) >>> 0, 8 | types.mapKey[field.keyType], field.keyType);
                        if (wireType === undefined) gen("types[%d].encode(%s[ks[i]],w.uint32(18).fork()).ldelim().ldelim()", index, ref); // can't be groups
                        else gen(".uint32(%d).%s(%s[ks[i]]).ldelim()", 16 | wireType, type, ref);
                        gen("}")("}");
                        ...
                        if (field.packed && types.packed[type] !== undefined) {
                            gen("if(%s&&%s.length&&m.hasOwnProperty(%j)){", ref, ref, field.name)("w.uint32(%d).fork()", (field.id << 3 | 2) >>> 0)("for(var i=0;i<%s.length;++i)", ref)("w.%s(%s[i])", type, ref)("w.ldelim()")("}");

                            // Non-packed
                        } else {
                            gen("if(%s!==undefined&&m.hasOwnProperty(%j)){", ref, field.name)("for(var i=0;i<%s.length;++i)", ref);
                            if (wireType === undefined) genTypePartial(gen, field, index, ref + "[i]");else gen("w.uint32(%d).%s(%s[i])", (field.id << 3 | wireType) >>> 0, type, ref);
                            gen("}");
                        }
                        ...
                        if (!field.required) {
                            ...
                            if (field.long) gen("if(%s!==undefined&&%s!==null&&m.hasOwnProperty(%j))", ref, ref, field.name);else if (field.bytes || field.resolvedType && !(field.resolvedType instanceof Enum)) gen("if(%s&&m.hasOwnProperty(%j))", ref, field.name);else gen("if(%s!==undefined&&m.hasOwnProperty(%j))", ref, field.name);
                        }

                        if (wireType === undefined) genTypePartial(gen, field, index, ref);else gen("w.uint32(%d).%s(%s)", (field.id << 3 | wireType) >>> 0, type, ref);
                    }
                }

                // oneofs
                for (var i = 0; i < /* initializes */mtype.oneofsArray.length; ++i) {
                    var oneof = mtype._oneofsArray[i];gen("switch(%s){", "m" + util.safeProp(oneof.name));
                    for (var j = 0; j < /* direct */oneof.fieldsArray.length; ++j) {
                        var field = oneof.fieldsArray[j],
                            type = field.resolvedType instanceof Enum ? "uint32" : field.type,
                            wireType = types.basic[type];
                        ref = "m" + util.safeProp(field.name);gen("case%j:", field.name);
                        if (wireType === undefined) genTypePartial(gen, field, mtype._fieldsArray.indexOf(field), ref);else gen("w.uint32(%d).%s(%s)", (field.id << 3 | wireType) >>> 0, type, ref);
                        gen("break");
                    }gen("}");
                }
            ...
            Field.prototype.resolve = function resolve() {
                ...
                if ((this.typeDefault = types.defaults[this.type]) === undefined) {
                    ...
                    if (this.resolvedType = scope.lookup(this.type, Type)) this.typeDefault = null;else if (this.resolvedType = scope.lookup(this.type, Enum)) this.typeDefault = this.resolvedType.values[Object.keys(this.resolvedType.values)[0]]; // first defined
                    else throw Error("unresolvable field type: " + this.type + " in " + scope);
                }
                ...
                if (this.options && this.options.packed !== undefined && this.resolvedType && !(this.resolvedType instanceof Enum)) delete this.options.packed;
            ...
            Namespace.prototype.lookup = function lookup(path, filterType, parentAlreadyChecked) {
                ...
                if (path[0] === "") return this.root.lookup(path.slice(1), filterType);
                // Test if the first part matches any nested object, and if so, traverse if path contains more
                var found = this.get(path[0]);
                if (found) {
                    if (path.length === 1) {
                        if (!filterType || found instanceof filterType) return found;
                    } else if (found instanceof Namespace && (found = found.lookup(path.slice(1), filterType, true))) return found;
                }
            ...
            Namespace.prototype.lookupService = function lookupService(path) {
                ...
                if (!found) throw Error("no such service");
                return found;
            };
            ...
            Namespace.prototype.lookupEnum = function lookupEnum(path) {
                ...
                if (!found) throw Error("no such enum");
                return found.values;
            };
            ...
            function parse(source, root, options) {
                ...
                function parseNumber(token, insideTryCatch) {
                    ...
                    if (base16Re.test(tokenLower)) return sign * parseInt(token, 16);
                    if (base8Re.test(token)) return sign * parseInt(token, 8);
                    if (numberRe.test(tokenLower)) return sign * parseFloat(token);
                    /* istanbul ignore next */
                ...
                function parseId(token, acceptNegative) {
                    ...
                    /* istanbul ignore next */
                    if (token.charAt(0) === "-" && !acceptNegative) throw illegal(token, "id");
                    if (base10NegRe.test(token)) return parseInt(token, 10);
                    if (base16NegRe.test(tokenLower)) return parseInt(token, 16);
                    /* istanbul ignore else */
                ...
                function parseType(parent, token) {
                    var name = next();
                    /* istanbul ignore next */
                    if (!nameRe.test(name)) throw illegal(name, "type name");
                    ...
                    if (skip("{", true)) {
                        while ((token = next()) !== "}") {
                            var tokenLower = lower(token);
                            if (parseCommon(type, token)) continue;
                            switch (tokenLower) {
                ...
                function parseField(parent, rule, extend) {
                    ...
                    if (!field.comment) field.comment = cmnt(trailingLine);
                    ...
                    if (!isProto3 && field.repeated) field.setOption("packed", false, /* ifNotSet */true);
                    parent.add(field);
                }
                ...
                function parseOneOf(parent, token) {
                    ...
                    if (skip("{", true)) {
                        ...
                        skip(";");
                        if (!oneof.comment) oneof.comment = cmnt(trailingLine);
                    }
                    parent.add(oneof);
                }
                ...
                function parseEnumValue(parent, token) {
                    ...
                    if (!parent.comments[name]) parent.comments[name] = cmnt(trailingLine);
                }
                ...
                function parseOption(parent, token) {
                    ...
                    /* istanbul ignore next */
                    if (!typeRefRe.test(name)) throw illegal(name, "name");
                ...
                function parseService(parent, token) {
                    ...
                    /* istanbul ignore next */
                    if (!nameRe.test(token)) throw illegal(token, "service name");
                ...
                function parseMethod(parent, token) {
                    ...
                    /* istanbul ignore next */
                    if (!nameRe.test(name)) throw illegal(name, "name");
                    var requestType, requestStream, responseType, responseStream;
                    skip("(");
                    ...
                    if (skip("{", true)) {
                        ...
                        skip(";");
                        if (!method.comment) method.comment = cmnt(trailingLine);
                    }
                    parent.add(method);
                }
                ...
                function parseExtension(parent, token) {
                    ...
                    /* istanbul ignore next */
                    if (!typeRefRe.test(reference)) throw illegal(reference, "reference");
                ...
                function fetch(filename, weak) {
                    ...
                    if (sync) {
                        ...
                        util.fetch(filename, function (err, source) {
                            ...
                            if (err) {
                                if (!weak) finish(err);else /* istanbul ignore next */if (!queued) // can't be covered reliably
                                        finish(null, self);
                                return;
                            }
                            process(filename, source);
            ...
            function tokenize(source) {
                ...
                function illegal(subject) {
                    return Error("illegal " + subject + " (line " + _line + ")");
                }
                ...
                function next() {
                    ...
                    var repeat, prev, curr, start, isComment;
                    do {
                        ...
                        if (charAt(offset) === "/") {
                            ...
                            if (charAt(offset) === "/") {
                                ...
                                if (isComment) setComment(start, offset - 1);
                                ++_line;
                                repeat = true;
                                ...
                                ++offset;
                                if (isComment) setComment(start, offset - 2);
                                repeat = true;
                            } else return "/";
                        }
                    } while (repeat);
                ...
                return {
                    ...
                    cmnt: function cmnt(trailingLine) {
                        var ret;
                        if (trailingLine === undefined) ret = commentLine === _line - 1 && commentText || null;else {
                            if (!commentText) peek();
            ...
            Object.defineProperties(Type.prototype, {
                ...
                ctor: {
                    ...
                    set: function set(ctor) {
                        if (ctor && !(ctor.prototype instanceof Message)) throw TypeError("ctor must be a Message constructor");
                        if (!ctor.from) ctor.from = Message.from;
                        this._ctor = ctor;
                    }
                }
            });
            ...
            Type.prototype.isReservedId = function isReservedId(id) {
                if (this.reserved) for (var i = 0; i < this.reserved.length; ++i) {
                    if (typeof this.reserved[i] !== "string" && this.reserved[i][0] <= id && this.reserved[i][1] >= id) return true;
                }return false;
            };
            ...
            Type.prototype.isReservedName = function isReservedName(name) {
                if (this.reserved) for (var i = 0; i < this.reserved.length; ++i) {
                    if (this.reserved[i] === name) return true;
                }return false;
            };
            ...
            util.toArray = function toArray(object) {
                var array = [];
                if (object) for (var keys = Object.keys(object), i = 0; i < keys.length; ++i) {
                ...
                if (field.resolvedType) {
                    if (field.resolvedType instanceof Enum) {
                        ...
                        for (var keys = Object.keys(field.resolvedType.values), j = 0; j < keys.length; ++j) {
                            gen("case %d:", field.resolvedType.values[keys[j]]);
                        }gen("break")("}");
                    } else gen("var e=types[%d].verify(%s);", fieldIndex, ref)("if(e)")("return%j+e", field.name + ".");
                } else {
                    switch (field.type) {
                        case "int32":
                        case "uint32":
            ...
            function verifier(mtype) {
                ...
                var gen = util.codegen("m")("if(typeof m!==\"object\"||m===null)")("return%j", "object expected");
                ...
                for (var i = 0; i < /* initializes */mtype.fieldsArray.length; ++i) {
                    ...
                    // map fields
                    if (field.map) {
                        gen("if(%s!==undefined){", ref)("if(!util.isObject(%s))", ref)("return%j", invalid(field, "object"))("var k=Object.keys(%s)", ref)("for(var i=0;i<k.length;++i){");
                        genVerifyKey(gen, field, "k[i]");
                        genVerifyValue(gen, field, i, ref + "[k[i]]")("}")("}");

                        // repeated fields
                    } else if (field.repeated) {
                        gen("if(%s!==undefined){", ref)("if(!Array.isArray(%s))", ref)("return%j", invalid(field, "array"))("for(var i=0;i<%s.length;++i){", ref);
                        genVerifyValue(gen, field, i, ref + "[i]")("}")("}");

                        // required or present fields
                    } else {
                        if (!field.required) {
                            if (field.resolvedType && !(field.resolvedType instanceof Enum)) gen("if(%s!==undefined&&%s!==null){", ref, ref);else gen("if(%s!==undefined){", ref);
                        }
            ...
            : function writeFloat_ieee754(value, buf, pos) {
                ...
                if (sign) value = -value;
                if (value === 0) writeFixed32(1 / value > 0 ? /* positive */0 : /* negative 0 */2147483648, buf, pos);else if (isNaN(value)) writeFixed32(2147483647, buf, pos);else if (value > 3.4028234663852886e+38) // +-Infinity
