...
(function (window, document, Laya) {
	...
	var MiniAdpter = function () {
		...
		MiniAdpter.getUrlEncode = function (url, type) {
			if (url.indexOf(".fnt") != -1) return "utf8";else if (type == "arraybuffer") return "";
			return "ascii";
		};
		...
		MiniAdpter.idx = 1;
		__static(MiniAdpter, ['nativefiles', function () {
	...
	var MiniFileMgr = function () {
		...
		MiniFileMgr.downFiles = function (fileUrl, encoding, callBack, readyUrl, isSaveFile, fileType) {
			...
			var downloadTask = MiniFileMgr.wxdown({ url: fileUrl, success: function success(data) {
					if (data.statusCode === 200) MiniFileMgr.readFile(data.tempFilePath, encoding, callBack, readyUrl, isSaveFile, fileType);
				}, fail: function fail(data) {
		...
		MiniFileMgr.downOtherFiles = function (fileUrl, callBack, readyUrl, isSaveFile) {
			...
			MiniFileMgr.wxdown({ url: fileUrl, success: function success(data) {
					if (data.statusCode === 200) {
						if ((MiniAdpter.autoCacheFile || isSaveFile) && readyUrl.indexOf("wx.qlogo.cn") == -1) MiniFileMgr.copyFile(data.tempFilePath, readyUrl, callBack);else callBack != null && callBack.runWith([0, data.tempFilePath]);
					}
				}, fail: function fail(data) {
					callBack != null && callBack.runWith([1, data]);
				} });
		...
		MiniFileMgr.downLoadFile = function (fileUrl, fileType, callBack, encoding) {
			fileType === void 0 && (fileType = "");
			encoding === void 0 && (encoding = "ascii");
			if (fileType == /*laya.net.Loader.IMAGE*/"image" || fileType == /*laya.net.Loader.SOUND*/"sound") MiniFileMgr.downOtherFiles(fileUrl, callBack, fileUrl, true);else MiniFileMgr.downFiles(fileUrl, encoding, callBack, fileUrl, true, fileType);
		};
		...
		MiniFileMgr.deleteFile = function (tempFileName, readyUrl, callBack, encoding, fileSize) {
			...
			MiniFileMgr.fs.unlink({ filePath: deleteFileUrl, success: function success(data) {
					...
					if (tempFileName != "") {
						var saveFilePath = MiniFileMgr.getFileNativePath(tempFileName);
						MiniFileMgr.fs.copyFile({ srcPath: tempFileName, destPath: saveFilePath, success: function success(data) {
		...
		__static(MiniFileMgr, ['fs', function () {
	...
	var MiniImage = function () {
		...
		MiniImage.onCreateImage = function (sourceUrl, thisLoader, isLocal, tempFilePath) {
			...
			if (thisLoader._type == "nativeimage") {
				...
				new HTMLImage.create(fileNativeUrl, { onload: onload, onerror: onerror, onCreate: function onCreate(img) {
						image = img;
	...
	var MiniInput = function () {
		...
		MiniInput.wxinputFocus = function (e) {
			...
			MiniAdpter.window.wx.showKeyboard({ defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: 'done', success: function success(res) {}, fail: function fail(res) {} });
  ...
  */
		__proto.load = function (url, type, cache, group, ignoreCache) {
			...
			if (Loader.parserMap[type] != null) {
				...
				if (Loader.parserMap[type] instanceof laya.utils.Handler) Loader.parserMap[type].runWith(this);else Loader.parserMap[type].call(null, this);
				return;
			};
