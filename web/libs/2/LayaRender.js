...
(function () {
  ...
  */
	LayaRender.body = function (render, body) {
		...
		if (bodyRender.sprite && bodyRender.sprite.texture) {
			...
			// 如果sprite不存在，则初始化一个
			if (!sprite) sprite = body.layaSprite = _createBodySprite(render, body);

			// 如果sprite未在显示列表，则添加至显示列表
			if (!container.contains(sprite)) container.addChild(sprite);
	...
	var _createBodyPrimitive = function _createBodyPrimitive(render, body) {
		...
		for (var k = body.parts.length > 1 ? 1 : 0; k < body.parts.length; k++) {
			...
			if (options.showAngleIndicator || options.showAxes) {
				...
				primitive.drawLine(part.position.x - body.position.x, part.position.y - body.position.y, (part.vertices[0].x + part.vertices[part.vertices.length - 1].x) / 2 - body.position.x, (part.vertices[0].y + part.vertices[part.vertices.length - 1].y) / 2 - body.position.y);
			}
		}
  ...
  */
	LayaRender.constraint = function (render, constraint) {
		...
		// 如果sprite不存在，则初始化一个
		if (!sprite) sprite = constraint.layaSprite = new Laya.Sprite();
