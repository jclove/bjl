...
(function (window, document, Laya) {
	...
	var Handler = laya.utils.Handler,
	    ...
	    MathUtil = laya.maths.MathUtil,
	    Matrix = laya.maths.Matrix;
	var Node = laya.display.Node,
	    Point = laya.maths.Point,
	    Rectangle = laya.maths.Rectangle,
	    Render = laya.renders.Render;
	var RenderContext = laya.renders.RenderContext,
	    Resource = laya.resource.Resource,
	...
	var AnimationParser02 = function () {
		...
		AnimationParser02.parse = function (templet, reader) {
			...
			for (var i = 0, n = AnimationParser02._BLOCK.count; i < n; i++) {
				...
				if (fn == null) throw new Error("model file err,no this function:" + index + " " + blockName);else fn.call();
			}
		};
		...
		__static(AnimationParser02, ['_BLOCK', function () {
 ...
 */
  ...
  */
		...
		__static(BoneSlot, ['_tempMatrix', function () {
 ...
 */
	...
	var IkConstraint = function () {
		...
		__static(IkConstraint, ['radDeg', function () {
 ...
 */
  ...
  */
			...
			if (!true) {
				...
				for (i = 0, o = 0, curve = 0; i < spacesCount; i++, o += 3) {
					...
					if (curve != prevCurve) {
						...
						} else this.computeWorldVertices2(target, boneList, curve * 6 + 2, 8, world, 0);
					}
		...
		__static(PathConstraint, ['_tempMt', function () {
 ...
 */
  ...
  */
		__proto.play = function (index, playbackRate, overallDuration, playStart, playEnd) {
			...
			if (this._templet.loaded) this._calculatePlayDuration();else this._templet.once( /*laya.events.Event.LOADED*/"loaded", this, this._onAnimationTempletLoaded);
			this._update(0);
		};
  ...
  */
		__getset(0, __proto, 'templet', function () {
			...
			if (!this.state === /*laya.ani.AnimationState.stopped*/0) this.stop(true);
			if (this._templet !== value) {
				this._templet = value;
				if (value.loaded) this._computeFullKeyframeIndices();else value.once( /*laya.events.Event.LOADED*/"loaded", this, this._onTempletLoadedComputeFullKeyframeIndices, [this._cachePlayRate, this._cacheFrameRate]);
			}
		});
  ...
  */
		__getset(0, __proto, 'state', function () {
			if (this._currentAnimationClipIndex === -1) return (/*laya.ani.AnimationState.stopped*/0
			);
			if (this._paused) return (/*laya.ani.AnimationState.paused*/1
			);
  ...
  */
		__getset(0, __proto, 'cachePlayRate', function () {
			...
			if (this._cachePlayRate !== value) {
				...
				if (this._templet) if (this._templet.loaded) this._computeFullKeyframeIndices();else this._templet.once( /*laya.events.Event.LOADED*/"loaded", this, this._onTempletLoadedComputeFullKeyframeIndices, [value, this._cacheFrameRate]);
			}
		});
  ...
  */
		__getset(0, __proto, 'cacheFrameRate', function () {
			...
			if (this._cacheFrameRate !== value) {
				...
				if (this._templet) if (this._templet.loaded) this._computeFullKeyframeIndices();else this._templet.once( /*laya.events.Event.LOADED*/"loaded", this, this._onTempletLoadedComputeFullKeyframeIndices, [this._cachePlayRate, value]);
			}
		});
 ...
 */
	...
	var SkinMeshCanvas = function (_super) {
		...
		__static(SkinMeshCanvas, ['_tempMatrix', function () {
			return this._tempMatrix = new Matrix();
		}]);
 ...
 */
	...
	var CacheAbleSkinMesh = function (_super) {
		...
		__static(CacheAbleSkinMesh, ['tempMt', function () {
			return this.tempMt = new Matrix();
		}]);
